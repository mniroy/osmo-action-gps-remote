...
esp_err_t ble_init() {
    ...
    /* Load saved camera MAC address from NVS if available */
    ble_load_connected_device_from_nvs();
...
/* ----------------------------------------------------------------
 *   NVS save/load for auto-reconnection
 * ---------------------------------------------------------------- */

#define NVS_NAMESPACE "dji_remote"
#define KEY_CAMERA_MAC "camera_mac"
#define KEY_CAMERA_NAME "camera_name"
...
void ble_save_connected_device_to_nvs(void) {
    ...
    // Save MAC address
    err = nvs_set_blob(my_handle, KEY_CAMERA_MAC, s_ble_profile.remote_bda, ESP_BD_ADDR_LEN);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error saving MAC address to NVS: %s", esp_err_to_name(err));
    }
    
    // Save Device Name
    if (strlen(s_remote_device_name) > 0) {
        err = nvs_set_str(my_handle, KEY_CAMERA_NAME, s_remote_device_name);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Error saving Device Name to NVS: %s", esp_err_to_name(err));
        }
    }
    ...
    nvs_close(my_handle);
    ESP_LOGI(TAG, "Saved camera MAC to NVS: %02X:%02X:%02X:%02X:%02X:%02X, name: %s",
...
bool ble_load_connected_device_from_nvs(void) {
    ...
    if (err != ESP_OK) {
        ESP_LOGI(TAG, "No NVS saved camera found (NVS open: %s)", esp_err_to_name(err));
        return false;
    }
    
    size_t mac_len = ESP_BD_ADDR_LEN;
    err = nvs_get_blob(my_handle, KEY_CAMERA_MAC, s_ble_profile.remote_bda, &mac_len);
    if (err != ESP_OK) {
        ESP_LOGI(TAG, "No MAC address blob found in NVS");
        nvs_close(my_handle);
        return false;
    }
    
    size_t name_len = sizeof(s_remote_device_name);
    err = nvs_get_str(my_handle, KEY_CAMERA_NAME, s_remote_device_name, &name_len);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "No Device Name string found in NVS, using empty");
        s_remote_device_name[0] = '\0';
    }
    
    nvs_close(my_handle);
    
    // Also copy to best_addr so reconnect can use it
    memcpy(best_addr, s_ble_profile.remote_bda, ESP_BD_ADDR_LEN);
    
    ESP_LOGI(TAG, "Loaded camera MAC from NVS: %02X:%02X:%02X:%02X:%02X:%02X, name: %s",
             s_ble_profile.remote_bda[0], s_ble_profile.remote_bda[1], s_ble_profile.remote_bda[2],
             s_ble_profile.remote_bda[3], s_ble_profile.remote_bda[4], s_ble_profile.remote_bda[5],
             s_remote_device_name);
